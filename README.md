...
Each EPUB also includes a generated SVG cover with the article title, article author, and source domain. Text on the cover is exported as SVG path geometry for Kindle compatibility. The cover author uses article frontmatter `author`, `byline`, or `creator` when present. The source domain is inferred from supported URL frontmatter such as `url`, `canonical_url`, `original_url`, or `link`.
